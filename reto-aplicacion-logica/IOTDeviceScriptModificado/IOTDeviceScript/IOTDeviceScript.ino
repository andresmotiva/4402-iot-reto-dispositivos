...
// Pin de la medida del fotoresistor
#define LUMI_PIN A0
...
const char MQTT_HOST[] = "3.228.13.99"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
bool checkMeasures(float t, float h, float l) {
  // Se comprueba si ha habido algún error en la lectura
    ...
    if (isnan(l)){
      Serial.println("Error obteniendo los datos del fotoresistor");
      return false;
    }
    return true;
}
...
String checkAlert() {
  ...
  if (alert.length() != 0) {
    ...
    if(alertTime < millis() && control_alert == false){
      alertTime = millis() + ALERT_DURATION * 1000;
      control_alert = true;
    }

    if ((millis() - alertTime) >= 0 ) {
      alert = "";
      alertTime = millis();
      control_alert = false;
     }
